...
export default async function handler(req, res) {
  ...
  const payload = {
    ...
    ExpressionAttributeNames: {
      ...
      "#priceHistory": "priceHistory", "#activity": "activity", "#category": "category"
    },
    ExpressionAttributeValues: {
      ":owner": formattedOwner, ":onSale": Number(0), ":saleId": Number(0), ":price": Number(0), ":saleType": Number(process.env.NEXT_PUBLIC_SALE_TYPE_NOT_EXTSTS),
      ":priceHistory": data.priceHistory, ":activity": data.activity, ":category": "null"
    },
    UpdateExpression:
      `set #owner = :owner, #onSale = :onSale, #saleId = :saleId, #price = :price, #saleType = :saleType, #priceHistory = :priceHistory, #activity = :activity, #category = :category`
  };
