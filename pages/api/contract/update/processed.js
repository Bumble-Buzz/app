...
export default async function handler(req, res) {
  ...
  if (!data.contractAddress) return res.status(400).json({ error: `contract address is invalid` });
  ...
  const payload = {
    ...
    Key: { 'contractAddress': ethers.utils.getAddress(data.contractAddress) },
    ExpressionAttributeNames: { "#isProcessed": "isProcessed" },
    ExpressionAttributeValues: { ":isProcessed": data.isProcessed },
    UpdateExpression: `set #isProcessed = :isProcessed`
  };
