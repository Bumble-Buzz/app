...
const IDENTIFIER = 'incentive_amount';
const DIALOG = { title: '', content: () => {} };

export default function IncentiveAmount({ isLoading, setLoading, account, setAccount }) {
  ...
  const depositAction = async (e, _value) => {
    ...
    try {
      ...
      // deposit incentives
      const val = await contract.depositMarketplaceIncentiveVault({ value: ethers.utils.parseEther(_value.toString()) });
